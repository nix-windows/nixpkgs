...
let
  zipIntBits = f: x: y:
    let
      ...
      intToBits = x:
        if x == 0 || x == -1 then
          ...
          let
            ...
            tailbits = (if x < 0 then x - 1 else x) / 2;  # x >> 1
          in
      ...
      bitsToInt = signum: l:
        if l == [] then
          (if sign == 0 then 0 else -1)
        else
          (builtins.head l) + (2 * (bitsToInt signum (builtins.tail l)));

      x-signum = if x < 0 then 1 else 0;
      y-signum = if y < 0 then 1 else 0;
      zipListsWith' = fst: snd:
        if fst==[] && snd==[] then
          []
        else if fst==[] then
          [(f x-signum            (builtins.head snd))] ++ (zipListsWith' []                  (builtins.tail snd))
        else if snd==[] then
          [(f (builtins.head fst) y-signum           )] ++ (zipListsWith' (builtins.tail fst) []                 )
        else
    ...
    in
      ...
      bitsToInt (f x-signum y-signum) (zipListsWith' (intToBits x) (intToBits y));
in
rec {
