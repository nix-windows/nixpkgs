{ lib, callPackage, fetchurl, fetchFromGitHub, stdenv, gcc6Stdenv }:
...
let
  ...
  missing-documentation-patch = fetchurl {
    ...
    url = "https://aur.archlinux.org/cgit/aur.git/plain/deny_missing_docs.patch?h=firefox-esr&id=03bdd01f9cf";
...
rec {
  ...
  firefox-esr-52 = (common rec {
    ...
    patches = [
      ...
      # libc 2.30 already has 'pid_t gettid()'
      ./firefox52-glibc2.30.patch
    ];
    ...
    stdenv = gcc6Stdenv; # gcc7 fails with "undefined reference to `__divmoddi4'"
    gtk3Support = false;
  };
  ...
  firefox-esr-60 = common rec {
    ...
    meta = common-meta // {
      ...
      broken = stdenv.buildPlatform.is32bit; # build fails with "out of memory"
    };
  };
